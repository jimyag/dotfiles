...
- 评论最小化需要仓库写入权限，并须取得对 `minimizeComment(OUTDATED)` 的单独授权；当前会话已有的该项授权在其范围内持续有效。此批准要求仅适用于评论最小化写入，不适用于只读 `gh api` 查询。写入只允许针对本次调用已记录的 `triggerNodeId`；未获授权或权限不足时保留评论并在汇报中说明，不因此重试触发命令。
...
5. 只修复由当前 PR 引入且有代码证据的问题。先用当前代码、测试和已有决定核对含义不清或相互冲突的意见；仍有会改变结果的未决歧义，或修复需要扩大 PR 范围时，请求用户判断，只暂停依赖该决定的修改，继续处理可独立完成的已授权事项。不得自行接受范围扩张。
6. 使用仓库规定的定向检查验证改动，分别记录本地验证和远程检查。先核对当前会话及仓库明确允许的验证命令，已有授权不重复询问。需要未获授权的命令时，先寻找能证明同一目标的已授权检查；没有合适替代才请求授权。等待期间继续不依赖该验证的工作；必要验证未获授权、未完成或失败时不得提交。
   - 评审意见涉及正确性、幂等、并发、API 行为或回归风险时，优先补充对应的聚焦回归场景或测试；不要用一次全量测试代替场景证据。
   - 遵循仓库的测试层级和运行约束；不要未经授权启动共享环境、运行高成本全量测试或把编译通过描述成真实运行面通过。
   - 若有 `test-scenarios.md`，把本轮修复、定向验证和未执行项映射到场景 ID，区分单元、集成、E2E 和人工证据。
...
- 已识别的评审者在当前来源分支版本上都没有可执行反馈，且没有待用户判断的意见。
- PR 来源分支版本被其他人更新，或本地存在会混入提交的无关改动。
- 没有可独立推进的已授权事项，继续工作依赖用户对未决歧义、评审者结论冲突或 PR 范围变更作出判断。
