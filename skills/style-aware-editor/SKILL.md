...
- 只有 `edit` 模式原地修改文件；`rewrite`、`detect` 和 `review` 返回文本或审查结果，不改动源文件。
...
- 默认不删除中文长篇公开文章中的整句。用户明确要求删除某类内容或允许结构性改写时，可删除对应句子；其他空洞句子列为删除建议。
- 处理发布说明、上线文案、变更日志、问题或 PR 文本、产品主张时，根据实际产物、差异、截图或源文档措辞，不凭空做泛化润色。
- 事实和代码保持准确。不编造名称、数字、引文、性能数据或轶事。
- 优先使用具体名词和动词，而非抽象框架。
- 删除仅为结构对称而添加的“非目标”“不做什么”或“范围外”章节。会影响实现、安全、兼容性或验收的排除项应保留，并就地改写为具体约束。
- 删除空洞过渡句，不换成另一句空洞过渡。
- 保留有用的重复，不为避免重复而轮换技术术语。
- 不要把每个段落都改成三段式结构。
- 删除聊天机器人残留，如“当然”“希望这有帮助”“让我们深入探讨”“值得注意的是”“总而言之”`in conclusion`（总结式收尾）、`let me know`（有需要请告诉我）。
- 删除与读者无关的代理过程旁白，例如“我正在使用某技能”“接下来我会调用工具”；仅在过程会影响结果可信度或复现方式时保留。
- 删除为了填满模板而出现的空章节、`无`/`不适用`占位和重复摘要；真实限制、风险和未完成项仍需保留。
...
1. 修改后的章节或已应用的改动。
2. 存在需要核实的事实或依赖用户偏好的位置时，说明具体风险。
3. 问题较多且汇总能帮助复核时，可补充简要说明。
