...
- TL;DR、风险、验证入口和 reviewer 关注点是否与实际 diff 一致；不要用泛化描述掩盖未验证的行为。
- 将核心逻辑、机械重命名、生成文件和格式化变更分开说明；如果改动过大，优先拆分而不是靠正文解释降低评审成本。
- 说明迁移顺序、rollout/回滚条件、兼容性影响和未覆盖场景；每个关键结论都指向 commit、测试、日志或运行面证据。
- 只有用户明确授权时才改写历史或 force-push。改写前记录原始 tree，改写后确认目标 tree 未发生意外变化。
