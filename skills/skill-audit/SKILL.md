...
5. 检查行为质量。
   ...
   - 检查是否要求向用户播报 skill 名称、内部工具或代理编排；只有这些信息影响可信度、复现或授权时才保留。
   - 检查输出模板是否强迫空章节、`none` 占位、正反对照、摘要复述或固定“风险/下一步”；模板应作为内容检查清单，只输出实际信息。
   - 声称重写改善质量或成本前，必须有基线对比。
   - 分别检查启动入口、最小验证闭环和文档命令是否能在真实仓库中运行；静态扫描通过不代表代理能完成一次小改动并验证结果。
   - 对脚本、MCP、hook 和远程命令检查是否读取敏感目录、外传数据、自动发布或执行破坏性操作；实际工作流不需要的权限不应写进 skill。
   - 对独立安装的技能做一次隔离 smoke check，确认发现路径、引用文件、工具假设和输出契约不依赖源仓库旁的未打包文件。
6. 按优先级整理问题。
   - 先修复打包损坏和行为冲突。
